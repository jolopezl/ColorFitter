void PlotLPEloss()
{
  ...
  TF1* fg11 = new TF1("fg11", "1/(2*[0]) * ((1-x)*([1] + 2*([2] - [4])) - TMath::Power([3],2)/(x*([1] + 2*([2]-[4]))))", 0.001, 0.9995);
  fg11->SetParName(0, "KAPPA");
  fg11->SetParName(1, "MP");
  fg11->SetParName(2, "NU");
  fg11->SetParName(3, "mT");
  fg11->SetParName(4, "ELOSS");
  fg11->SetParameter(0, 1);
  fg11->FixParameter(1, 0.938);
  fg11->FixParameter(2, 12.4);
  // fg1->FixParameter(3, 0.14);
  fg11->FixParameter(3, mass_trans);
  fg11->SetParameter(4, 0.0);
  fg11->SetParLimits(4, 0.0, 1.0);
  ...
  model->Fit("fg11", "EMN0", "", 0.25, 0.95);
  fg11->SetLineStyle(kDashed);
  fg11->SetLineWidth(4);
  ...
  cout << "******** with energy loss " << endl;
  cout << "Chi^2 = " << fg11->GetChisquare() << endl;
  cout << "NDF = " << fg11->GetNDF() << endl;
  cout << "Chi^2/NDF = " << fg11->GetChisquare() / fg11->GetNDF() << endl;
  cout << "p-value = " << fg11->GetProb() << endl;
  cout << "********" << endl;
