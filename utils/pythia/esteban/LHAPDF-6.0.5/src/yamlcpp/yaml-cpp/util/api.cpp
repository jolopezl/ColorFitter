...
{
	{
		...
		// move-like semantics
		LHAPDF_YAML::Value root = LHAPDF_YAML::Parse("test.yaml");
...

	{
		...
		LHAPDF_YAML::Value root = "Hello";  // Hello
		root = LHAPDF_YAML::Sequence();     // []
		root[0] = 0;                 // [0]
		root[2] = "two";             // [0, ~, two]  # forces root[1] to be initialized to null

		LHAPDF_YAML::Value other = root;    // both point to the same thing
	...
	{
		LHAPDF_YAML::Value map;             // ~
		map[3] = 1;                  // {3: 1}  # auto-constructs a map, *not* a sequence
		
		LHAPDF_YAML::Value seq;             // ~ 
		seq = LHAPDF_YAML::Sequence();      // []
		seq[3] = 1;                  // [~, ~, ~, 1]
	}
	...
	{
		LHAPDF_YAML::Value node;
		LHAPDF_YAML::Value subnode = node["key"]; // 'subnode' is not instantiated ('node' is still null)
		subnode = "value";           // {key: value}  # now it is
		LHAPDF_YAML::Value subnode2 = node["key2"];
	...
	{
		LHAPDF_YAML::Value seq = LHAPDF_YAML::Sequence();
		seq[0] = "zero";             // [zero]
	...
	{
		LHAPDF_YAML::Value root;
		root.push_back("zero");
		root.push_back("one");
		root.push_back("two");
		LHAPDF_YAML::Value two = root[2];
	...
	
	{
		...
		LHAPDF_YAML::Value sub = root[1];   // &1 [bye, [hi, *1]]  # sub
