...
def main():
    print("Model results")
    f = ROOT.TFile.Open("OutputROOT.20200218.BL30.root", "READ")

    fig, axs = plt.subplots(2, 4, sharey='row', sharex='col',constrained_layout=True,figsize=(9,4.5))
    ...
    for i in range(4):
        ...
        axs[0,i].errorbar(f.Get(graph_name).GetX(), f.Get(graph_name).GetY(), f.Get(graph_name).GetEY(), marker="o", linestyle="",color='black',zorder=2)
        axs[0,i].plot(f.Get(extrapolation_name).GetX(), f.Get(extrapolation_name).GetY(), "b-",zorder=1)
        ...
        axs[1,i].errorbar(f.Get(graph_name).GetX(), f.Get(graph_name).GetY(), f.Get(graph_name).GetEY(), marker="o", linestyle="", color='black',zorder=2)
        axs[1,i].plot(f.Get(extrapolation_name).GetX(), f.Get(extrapolation_name).GetY(), "r-",zorder=1)
