...
void analysis(int opt = 1, int zbin = 1)
{
  ...
  tg_model_up->SetLineColor(kCyan);
  tg_model_up->SetLineStyle(kDashed);
  tg_model_down->SetLineColor(kCyan);
  tg_model_down->SetLineStyle(kDashed);
  ...
  TF1* func = new TF1("func", "gaus", -1, 2);
  ...
  if (opt == 1) {
    h_distributions_aux = new TH1F("h_distributions_aux", ";R_{M};Entries", 50, 0.4, 1.1);
  } else {
    h_distributions_aux = new TH1F("h_distributions_aux", ";p_{T}^2 [GeV^{2}];Entries", 50, -0.05, 0.05);
  }
  ...
  for (int i = 0; i < nbins; ++i) {
    ...
    if (opt == 1) {
      tree->Project("h_distributions_aux", "PT2", Form("A13 > %f && A13 < %f && zbin==%d && StatusCode == 0", cut_lo, cut_hi, zbin));
    } else {
      tree->Project("h_distributions_aux", "RM", Form("A13 > %f && A13 < %f && zbin==%d && StatusCode == 0", cut_lo, cut_hi, zbin));
    }
    ...
    if (opt == 1) {
      h_distributions[i] = new TH1F(hist_name, ";R_{M};Entries", 50, h_lo, h_hi);
    } else {
      h_distributions[i] = new TH1F(hist_name, ";p_{T}^{2} [GeV^{2}];Entries", 50, h_lo, h_hi);
    }
    if (opt == 1) {
      tree->Project(hist_name, "PT2", Form("A13 > %f && A13 < %f && zbin==%d && StatusCode == 0", cut_lo, cut_hi, zbin));
    } else {
      tree->Project(hist_name, "RM", Form("A13 > %f && A13 < %f && zbin==%d && StatusCode == 0", cut_lo, cut_hi, zbin));
    }
    ...
    for (int ib = 1; ib < 101; ++ib) {
      if (temp->GetBinContent(ib) != 0) {
        xmin = temp->GetBinCenter(ib);
        break;
      }
    }
    for (int ib = 100; ib > 0; --ib) {
      if (temp->GetBinContent(ib) != 0) {
        xmax = temp->GetBinCenter(ib);
        break;
      }
    }
    ...
    myText(0.2, 0.7, kBlack, Form("#splitline{#splitline{Gaussian fit}{mean = %.3f}}{#splitline{sigma = %.3f}{#chi^{2}/ndf = %.1f/%d}}", func->GetParameter(1), func->GetParameter(2), func->GetChisquare(), func->GetNDF()), 16);
  ...
  TF1* ff_down = nullptr;
  if (opt == 1) {
    ff_up = new TF1("ff_up", "[0] + [1]*x + [2]*pow(x,2)", 2.5, 6);
    ff_up->SetLineStyle(kDashed);
    ff_down = new TF1("ff_down", "[0] + [1]*x + [2]*pow(x,2)", 2.5, 6);
    ff_down->SetLineStyle(kDashed);
  } else {
    ff_up = new TF1("ff_up", "[0]*TMath::Exp(-[1]*x)", 2.5, 6);
    ff_up->SetLineStyle(kDashed);
    ff_down = new TF1("ff_down", "[0]*TMath::Exp(-[1]*x)", 2.5, 6);
    ff_down->SetLineStyle(kDashed);
  }
