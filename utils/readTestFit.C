...
void readTestFit(std::string filename) {
    ...
    if (foutname.size() > 0) {foutname.resize(foutname.size() - 4);}
    foutname = foutname + ".root";
    TFile *fout = TFile::Open(foutname.c_str(), "RECREATE");

    double zbin[4]; double zbinw[4] = {0.20/2,0.22/2,0.22/2,0.16/2}; // Approx.
    ...
    while (std::getline(file,line)) {
        ...
        q0[i] = std::stod(words.at(1)); q0_err[i] = std::stod(words.at(1+6));
        q0[i] = q0[i]/factor; q0_err[i] = q0_err[i]/factor;
        lp[i] = std::stod(words.at(2)); lp_err[i] = std::stod(words.at(2+6));
        cs[i] = std::stod(words.at(3)); cs_err[i] = std::stod(words.at(3+6));
    ...
    gr[2]->SetName("tg_cs"); gr[2]->SetTitle("tg_cs;z_{h};#sigma [mbarn]");
