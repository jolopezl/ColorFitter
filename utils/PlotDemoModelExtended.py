...
def main():
    ...
    fs = ROOT.TFile.Open("DemoPlots.Sig.root", "READ")

    fig, axs = plt.subplots(3, 3, sharex='col', constrained_layout=True,figsize=(9,6))
    ...
    axs[1,1].set(xlabel='', ylabel=r'$\Delta \langle p_\mathrm{T}^2\rangle$ (GeV$^2$)')
    axs[0,2].set(xlabel='', ylabel='$R_\mathrm{M}$')
    axs[1,2].set(xlabel='', ylabel='$R_\mathrm{M}$')
...
# if (A == (int) 131.293)  {return 5.54713;}
    ...
    for sig in sigmas:
        ...
        axs[2,1].plot(fs.Get(graph_name).GetX(), fs.Get(graph_name).GetY(),
                    current_style, label='$\sigma = '+str(sig)+'$ (mbar)')
        # Multiplicity ratio
        graph_name = "RM_lp_"+str(sig)
        axs[2,2].plot(fs.Get(graph_name).GetX(), fs.Get(graph_name).GetY(),
                    current_style, label='$\sigma = '+str(sig)+'$ (mbar)')

    axs[2,1].set(
        xlabel='$A^{1/3}$', ylabel=r'$\Delta \langle p_\mathrm{T}^2\rangle$ (GeV$^2$)')
    axs[2,2].set(xlabel='$A^{1/3}$', ylabel='$R_\mathrm{M}$')
    axs[2,1].legend(title="For $L_{c}$ = 5 (fm)")

    axs[2,1].set_ylim(0.001,0.023)
