...
def retrieveTGraphErrors(i,par,nop,aux):
  ...
  for i in range(4):
    ...
    # normalize y*[i]
    yval[i] = yval[i]/factor/(xval[i]**2)
    yerr[i] = yerr[i]/factor
...
xlabel = "z"
# L_P configuration
ylabel_up = "#hat{q}/z^{2} [GeV^{2}/fm]_{3P}"
ylabel_down = "#hat{q}/z^{2} [GeV^{2}/fm]_{4P}"
parameter = "pT2/lp"
...
def job(fileref,fout,nuclei_name):
  ...
  line0.Draw("SAME")
  # text1.DrawLatex(0.2,0.15,"#hat{q} = #Delta#LTp_{t}^{2}#GT/l_{p}")
...
job(0,"test_a_fig06_neon.pdf","Neon")
job(1,"test_b_fig06_krypton.pdf","Krypton")
job(2,"test_c_fig06_xenon.pdf","Xenon")