...
bool OutputResultsToFile(const std::string model, std::vector<myResult> resultCont)
{
  ...
  for (int i = 0; i < fNzbins; ++i) {
    ...
    zErr[i] = 0;
    q0[i] = resultCont.at(i).m_qhat;
    q0Err[i] = resultCont.at(i).m_qhat_err;
    lp[i] = resultCont.at(i).m_lp;
    lpErr[i] = resultCont.at(i).m_lp_err;
    sigma[i] = resultCont.at(i).m_sigma_ph;
    ...
    c2Err[i] = resultCont.at(i).m_c2_err;
    cascade[i] = resultCont.at(i).m_cascade;
    cascade_err[i] = resultCont.at(i).m_cascade;
  ...
  TGraphErrors* tg_q0 = new TGraphErrors(fNzbins, z, q0, zErr, q0Err);
  tg_q0->SetName("tg_q0");
  tg_q0->SetTitle(";#it{z};#it{q}_{0} (GeV^{2}fm^{2})");
  TGraphErrors* tg_lp = new TGraphErrors(fNzbins, z, lp, zErr, lpErr);
  tg_lp->SetName("tg_lp");
  tg_lp->SetTitle(";#it{z};#it{L}_{p} (fm)");
  TGraphErrors* tg_sigma = new TGraphErrors(fNzbins, z, sigma, zErr, sigmaErr);
  tg_sigma->SetName("tg_sigma");
  ...
  for (int i = 0; i < fNzbins; ++i) {
    ...
    tg_data_pT[i] = &(resultCont.at(i).m_tg_data_pT);
    tg_data_pT[i]->SetName(Form("tg_data_pT_%d", i));
    tg_data_pT[i]->SetTitle(title.c_str());
    tg_model_pT[i] = &(resultCont.at(i).m_tg_pT);
    tg_model_pT[i]->SetName(Form("tg_model_pT_%d", i));
    tg_model_pT[i]->SetTitle(title.c_str());
    tg_model_pT_extrapolation[i] = &(resultCont.at(i).m_tg_pT_extrapolation);
    tg_model_pT_extrapolation[i]->SetName(Form("tg_model_pT_extrapolation_%d", i));
    tg_model_pT_extrapolation[i]->SetTitle(title.c_str());
    tg_model_pT_extrapolation_up[i] = &(resultCont.at(i).m_tg_pT_extrapolation_up);
    tg_model_pT_extrapolation_up[i]->SetName(Form("tg_model_pT_extrapolation_up_%d", i));
    tg_model_pT_extrapolation_up[i]->SetTitle(title.c_str());
    tg_model_pT_extrapolation_down[i] = &(resultCont.at(i).m_tg_pT_extrapolation_down);
    tg_model_pT_extrapolation_down[i]->SetName(Form("tg_model_pT_extrapolation_down_%d", i));
    tg_model_pT_extrapolation_down[i]->SetTitle(title.c_str());
    title = ";A^{1/3};R_{M}";
    tg_data_Rm[i] = &(resultCont.at(i).m_tg_data_Rm);
    tg_data_Rm[i]->SetName(Form("tg_data_Rm_%d", i));
    tg_data_Rm[i]->SetTitle(title.c_str());
    tg_model_Rm[i] = &(resultCont.at(i).m_tg_Rm);
    tg_model_Rm[i]->SetName(Form("tg_model_Rm_%d", i));
    tg_model_Rm[i]->SetTitle(title.c_str());
    tg_model_Rm_extrapolation[i] = &(resultCont.at(i).m_tg_Rm_extrapolation);
    tg_model_Rm_extrapolation[i]->SetName(Form("tg_model_Rm_extrapolation_%d", i));
    tg_model_Rm_extrapolation[i]->SetTitle(title.c_str());
    tg_model_Rm_extrapolation_up[i] = &(resultCont.at(i).m_tg_Rm_extrapolation_up);
    tg_model_Rm_extrapolation_up[i]->SetName(Form("tg_model_Rm_extrapolation_up_%d", i));
    tg_model_Rm_extrapolation_up[i]->SetTitle(title.c_str());
    tg_model_Rm_extrapolation_down[i] = &(resultCont.at(i).m_tg_Rm_extrapolation_down);
    tg_model_Rm_extrapolation_down[i]->SetName(Form("tg_model_Rm_extrapolation_down_%d", i));
    tg_model_Rm_extrapolation_down[i]->SetTitle(title.c_str());
    title = ";A^{1/3};#LT #rho #GT";
    tg_average_density[i] = &(resultCont.at(i).m_tg_average_density);
    tg_average_density[i]->SetName(Form("tg_average_density_%d", i));
    tg_average_density[i]->SetTitle(title.c_str());
    title = ";A^{1/3};#LT #sigma exp(-#sigma#int#rhodz) #rho(z_{0}+l) #GT";
    tg_multiplicity_density[i] = &(resultCont.at(i).m_tg_multiplicity_density);
    tg_multiplicity_density[i]->SetName(Form("tg_multiplicity_density_%d", i));
    tg_multiplicity_density[i]->SetTitle(title.c_str());
